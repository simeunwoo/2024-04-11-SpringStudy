 ...
 */
/*
	@Controller : 화면 지정 => 어떤 화면을 보여줄 지 선택
	              ~~~~~~~ 출력할 데이터 전송 : Model
	              			addAttribute()
	...
	사용하는 형식 => 스프링에서 지정
		...
		= 리턴형
			String : forward, redirect
				forward : 데이터 전송
				redirect : 이전에 등록된 화면 이동
					return "redirect:list.do"
					=> _ok.do
				return "경로명/파일명"
			void : 다운로드
		= 매개 변수 : getParameter() => 매개 변수로 처리
			=> DispatcherServlet에서 매개 변수 설정
			=> ~VO 단위로 값을 받는다 => insert/update/join...
			   ================= 커맨드 객체
			=> 모든 데이터 : String => 해당 데이터형
			   =================
			   값이 없는 경우 : String => page를 받는 경우
			                          검색어
			                ------------------------ 디폴트 설정
			=> String[] : checkbox를 이용해서 값을 받는 경우
			=> 내장 객체
				HttpServletRequest : 사용 금지를 권장
					=> 단점은 사용자 IP, PORT 확인이 가능
					=> 사용 시 : Cookie 값 읽기
				HttpServletResponse
					=> 사용 시 : Cookie 저장 => 브라우저로 전송
					=> 다운로드
				...
				=> Cookie는 내장 객체가 아니다 (request를 이용)
			=> 매개 변수는 순서 없이 설정이 가능
		= 메소드명 : 자유롭게 => URL 주소만 매칭
	...
	# 스프링에서 주로 사용되는 패키지
		...
		com.sist.manager
			=> 추천, 크롤링, 외부 데이터 읽기 => OpenAI
			=> 날씨, 실시간 검색어 ex) Naver, Kakao
		com.sist.web : Model => 조립 => 결과값 전송
		com.sist.intercept : 중간에 처리 가능
		
			사용자 요청 (.do) ===> DispatcherServlet ===> HandlerMapping
				===> com.sist.web (@GetMapping, @PostMapping, @RequestMapping)
				===> URL 주소를 이용하여 메소드를 호출
				        | 결과값 (Model)
				     ViewResolver
				        | request로 변환
				       JSP
			...
			intercept 가능 구간 3곳
			1) Dispatcher -> HandlerMapping 사이
			2) URL 주소 -> ViewResolver 사이 결과값(Model) 구간
			3) ViewResolver -> JSP 사이 request 구간
			
		com.sist.aop : 공통으로 사용 => 요청 처리 수행 시간
			=> footer에 출력 : 자동화
		com.sist.commons : 예외 처리를 한번에 처리
	...
	# 패키지 안에서 사용하는 클래스 => 관리하는 영역 (스프링)
		관리 : 클래스 메모리 할당 =====> 소멸 (메모리 해제)
		
	# DispatcherServlet
	1) @WebServlet("*.do) => 스프링에는 존재하지 않는다
		=> web.xml에 등록
			web.xml : 톰캣에서 읽어가는 파일
			=> 서블릿과 JSP는 톰캣에 의하여 구동
			   ========== 찾는 방법 : URL
			=> request를 사용하지 않는다
				=> 한글 변환 => 등록
 */
...
public class DataBoardController {
	...
	public String databoard_list(String page,Model model)
	{
		...
		return "databoard/list"; // .jsp를 붙이면 404 오류 발생
	}
	...
	{
		String en=encoder.encode(vo.getPwd()); // 비밀 번호 암호화
		vo.setPwd(en);
		
		String path="c:\\spring_upload";
		List<MultipartFile> list=vo.getFiles();
		if(list==null) // 업로드되지 않았을 경우
		{
			vo.setFilename("");
			vo.setFilesize("");
			vo.setFilecount(0);
